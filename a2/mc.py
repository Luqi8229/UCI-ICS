...
def create_new_profile(path, info, admin):
    fileName = None
    if info[0] != "-n":
        if admin is True:
            print("Invalid command.")
    ...
    try:
        ...
        if admin is True:
            ...
            fileName = ui.prompt_info("Please give a name for the file", admin)
    if fileName.endswith(".dsu") is False:
        fileName += ".dsu"
    print(fileName)
    dsupath = path / fileName
    print(dsupath)
    if dsupath.exists():
        print("File already exists.")
        if admin is False:
            ui.print_admin_lines("Loading file...", admin) 
            open_DSU_file(dsupath, admin)
    else:
        if admin is False:
            ps.create_new_file(path, info)
            user, pswd, biop = ui.get_profile_info(admin)
            profile = Profile(dsuserver = str(dsupath), username = user, password = pswd, bio =biop)
            profile.save_profile(str(dsupath))
            ...
            if "-f" in options:
                ...
                try:
                    ...
                    if admin is True:
                        ...
                        select = ui.prompt_info("Please enter a filename:", admin)
                if admin is False:
                    while "." not in select:
                ...
                try:
                    ...
                    if admin is True:
                        ...
                        ext = ui.prompt_info("Please enter an extension:", admin)
                        options.append(ext)
